 ...
 * @subpackage Mwb_Cf7_Integration_With_Gsheets/admin
 */
 ...
 */
class Mwb_Cf7_Integration_With_Gsheets_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in Mwb_Cf7_Integration_With_Google_Sheets_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Mwb_Cf7_Integration_With_Google_Sheets_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		if ( $this->is_valid_screen() ) {
			wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/mwb-cf7-integration-with-gsheets-admin.min.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_screen() ) {
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/mwb-cf7-integration-with-gsheets-admin.min.js', array( 'jquery' ), $this->version, false );
			...
			$ajax_data = array(
				...
				'feedBackText'  => esc_html__( 'Back to feeds', 'mwb-cf7-integration-with-google-sheets' ),
				'isPage'        => isset( $_GET['tab'] ) ? sanitize_text_field( wp_unslash( $_GET['tab'] ) ) : '', // phpcs:ignore
				'criticalError' => esc_html__( 'Internal server error', 'mwb-cf7-integration-with-google-sheets' ),
				'trashIcon'     => MWB_CF7_INTEGRATION_WITH_GSHEETS_URL . 'admin/images/trash.svg',
				'api_key_image' => MWB_CF7_INTEGRATION_WITH_GSHEETS_URL . 'admin/images/api.png',
			);
	...
	public function mwb_cf7_integration_submenu_cb() {
		require_once MWB_CF7_INTEGRATION_WITH_GSHEETS_DIRPATH . 'admin/partials/mwb-cf7-integration-with-' . $this->crm_slug . '-admin-display.php';
	}
	...
	public function mwb_cf7_integration_admin_init_process() {
		...
		if ( 'gsheets' != $this->crm_slug ) { // phpcs:ignore
			return;
		}
		...
		if ( ! empty( $_GET['mwb-cf7-integration-perform-auth'] ) ) {
			...
			if ( ! isset( $_GET['state'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['state'] ) ), 'mwb_cf7_' . $this->crm_slug . '_state' ) ) {
				wp_die( 'The state is not correct from Google Server. Try again.' );
			}
	...
	public function add_plugin_headings() {
		...
		$headings = array(
			'name'    => esc_html__( 'MWB CF7 Integration with Google Sheets', 'mwb-cf7-integration-with-google-sheets' ),
			'version' => MWB_CF7_INTEGRATION_WITH_GSHEETS_VERSION,
		);
	...
	public function mwb_cf7_integration_add_deactivation_screens( $valid_screens = array() ) {
		...
		if ( is_array( $valid_screens ) ) {
			...
			// Push your screen here.
			array_push( $valid_screens, 'mwb-cf7-integration-with-google-sheets' );
		}
	...
	public static function pro_dependency_check() {
		...
		if ( mwb_gs_cf7_is_plugin_active( 'cf7-integration-with-google-sheets/cf7-integration-with-google-sheets.php' ) ) {
			...
			if ( class_exists( 'Cf7_Integration_With_Gsheets_Admin' ) ) {
				return true;
			}
		}
	...
	public static function version_compatibility_check() {
		...
		if ( self::pro_dependency_check() ) {
			...
			if ( defined( 'CF7_INTEGRATION_WITH_GOOGLE_SHEETS_VERSION' ) && version_compare( CF7_INTEGRATION_WITH_GOOGLE_SHEETS_VERSION, '1.0.0' ) < 0 ) {
	...
	public function mwb_cf7_integration_deactivate_pro_plugin() {
		...
		deactivate_plugins( 'cf7-integration-with-google-sheets/cf7-integration-with-google-sheets.php' );
	}
	...
	public function mwb_cf7_integration_deactivate_pro_admin_notice() {
		...
		$pro = esc_html__( 'CF7 Integration with Google Sheets', 'mwb-cf7-integration-with-google-sheets' );
		$org = esc_html__( 'MWB CF7 Integration with Google Sheets', 'mwb-cf7-integration-with-google-sheets' );
			...
			<div class="notice notice-error is-dismissible mwb-notice">
				<p>
					...
					echo sprintf(
						...
						esc_html__( '%1$s is deactivated, Please Update the PRO version as this version is outdated and will not work with the current %2$s Org version', 'mwb-cf7-integration-with-google-sheets' ),
	...
	public static function is_pro_available_and_active() {
		$result   = false;
		$crm_name = Mwb_Cf7_Integration_With_Gsheets::get_current_crm();
