...
jQuery( document ).ready( function() {
	...
	const crm          = mwb_cf7_integration_ajax_data.crm
	const ajaxUrl      = mwb_cf7_integration_ajax_data.ajaxUrl;
	const nonce        = mwb_cf7_integration_ajax_data.ajaxNonce;
	const action       = mwb_cf7_integration_ajax_data.ajaxAction;
	const error        = mwb_cf7_integration_ajax_data.criticalError;
	const feedBackLink = mwb_cf7_integration_ajax_data.feedBackLink;
	const feedBackText = mwb_cf7_integration_ajax_data.feedBackText;
	const isPage       = mwb_cf7_integration_ajax_data.isPage;
	const trashIcon    = mwb_cf7_integration_ajax_data.trashIcon;
	const apiKeyImg    = mwb_cf7_integration_ajax_data.api_key_image;
	...
	const enableDataTable = () => {
		...
		let ajax_url = jQuery( '#mwb-' + crm + '-cf7-logs' ).attr( 'ajax_url' );
		ajax_url     = ajax_url + '?action=get_datatable_logs';

		jQuery( '#mwb-' + crm + '-cf7-table' ).DataTable( {
			"processing": true,
			"serverSide": true,
			"ajax"      : ajax_url,
			"scrollX"   : true,
	 ...
	 const validateApi = ( searchParams ) => {
		if ( searchParams.has( 'mwb_auth' )  && searchParams.has( 'page' ) ) {
			if ( ( searchParams.get( 'mwb_auth' ) == 1 ) ) {
				if ( jQuery( '#mwb-cf7-auth_wrap' ).length > 0 ) {
	...
	const defaultFeedTitle = () => {
		...
		if ( jQuery( 'body' ).hasClass( 'post-type-mwb_' + crm + '_cf7' ) ) {
	...
	const hideFeedEditOptions = () => {
		...
		/* Display filters metabox only if valid CF7 form */
		let form = jQuery( '#mwb-' + crm + '-cf7-select-form' ).val();
		if ( '-1' == form ) {
			jQuery( '#mwb_' + crm + '_cf7_feeds_condition_meta_box' ).slideUp();
		} else {
			jQuery( '#mwb_' + crm + '_cf7_feeds_condition_meta_box' ).slideDown();
		}
	...
	const display_tooltip = () => {
		...
		jQuery( '.mwb_' + crm + '_cf7_tips' ).tipTip( args );
  	}
	 ...
	 * @return  void
	 */
	 const triggerSetupDocs = () => {

		Swal.fire({
			title: 'How to get the API keys?',
			html:
			'<ul class="mwb_list_points">' +
			'<li>Go to <a target="_blank" href="https://console.developers.google.com/">Google Developer Console</a> & select project or Create new one.</li>' +
			'<li>Go to Library and Select Google Sheets API  and Enable it, do the same for Google drive API.</li>' +
			'<li>Go to Oauth Consent Screen and enter name of Application (e.g Demo App ).</li>' +
			'<li>Go to Credentials -> New Credentials -> Select Oauth Client ID.</li>' +
			'<li>Select Web application as Application Type, Enter name -> Enter Authorized redirect URIs -> Create.</li>' +
			'<li>Copy Client ID and Client Secret.</li>' +
			'</ul>' + 
			'<img style=" width: 100%; border: 2px solid;" src="' + apiKeyImg + '">',
	...
	/**
	 * Close Reathorize popup
	 * 
	 * @since     1.0.0
	 * @returns   void
	 */
	const hideReauthPopup = () => {
		jQuery( '.mwb-reauth__body' ).addClass( 'row-hide' );
	}
	...
	const updateSelectOptions = ( selectId, selectOptions ) => {
		...
		let	html = '<option value="-1">Select Google Sheets</option>';
	...
	const initiliazeFeedForm = () => {
		...
		if ( '-1' != object && '-1' != form && '' != post_id && 'mwb_' + crm + '_cf7' == post_type ) {
		...
		if ( jQuery( 'body' ).hasClass( 'post-type-mwb_' + crm + '_cf7' ) ) {
			if ( jQuery( '#mwb-' + crm + '-enable-add-condition' ).is( ':checked' ) ) {
				jQuery( '.mwb-initial-filter' ).find( '.condition-value-field' ).attr( 'required', true );
			} else {
				jQuery( '.mwb-initial-filter' ).find( '.condition-value-field' ).removeAttr( 'required' );
			}
		}
	...
	const populateFeedFrom = ( data ) => {
		...
		let button = jQuery( '#mwb-' + crm + '-refresh-fields' ).find( 'span' );
		...
		result.then( ( response ) => {
			...
			if ( true == response.status ) {
				if ( response.data.hasOwnProperty( 'html' ) ) {
					jQuery('.mwb_cf7_integration__feeds-wrap').nextAll('.mwb-content-wrap').slideUp();
					jQuery('.mwb_cf7_integration__feeds-wrap').nextAll('.mwb-content-wrap').remove();
					jQuery('.mwb-feed__select-object').closest( 'div.mwb_cf7_integration__feeds-wrap' ).parent().append( response.data.html );
	...
	/**
	 * Reset Feed if not a valid CF7 form.
	 * 
	 * @since      1.0.0
	 * @returns    void.
	 */
	const resetFeedForm = () => {
		...
		jQuery('.mwb_' + crm + '_cf7__feeds-wrap').nextAll('.mwb-content-wrap').slideUp();
		jQuery('.mwb_' + crm + '_cf7__feeds-wrap').nextAll('.mwb-content-wrap').remove();
		jQuery( '#mwb_' + crm + '_cf7_feeds_condition_meta_box' ).slideUp();
	}
	 ...
	 */
	 const createAddNoteField = () => {
		...
		if ( jQuery( '#mwb_' + crm + '_cf7_feeds_note_meta_box' ).length == 0 ) {
			return;
		}
	 ...
	 * @return void
	 */
	const loadIntegrationFields = () => {
		let method = jQuery( 'input[name="mwb_account[integration_method]"]:checked' ).val();
		
		switch( method ) {
			case 'api':
				jQuery( '.mwb_sf_cf7_table' ).find( '.mwb-api-fields' ).slideDown();
				jQuery( '.mwb_sf_cf7_table' ).find( '.mwb-web-fields' ).slideUp();
				break;

			case 'web':
				jQuery( '.mwb_sf_cf7_table' ).find( '.mwb-api-fields' ).slideUp();
				jQuery( '.mwb_sf_cf7_table' ).find( '.mwb-web-fields' ).slideDown();
				break;
		}
	}
	 ...
	 * 
	 * @param    string module  Google module.
	 * @since    1.0.1
	 * @returns  void
	 */
	const manageAssociationFields = ( objects = false ) => {
		...
		if ( jQuery( 'body' ).hasClass( 'post-type-mwb_' + crm + '_cf7' ) ) {
			...
			jQuery( '#mwb_' + crm + '_cf7_feeds_associate_account_meta_box' ).css( 'display', 'none' );
			jQuery( '#mwb_' + crm + '_cf7_feeds_associate_contact_meta_box' ).css( 'display', 'none' );
			jQuery( '#mwb_' + crm + '_cf7_feeds_associate_campaign_meta_box' ).css( 'display', 'none' );

			//get dependent feeds.
			let dependent = jQuery( '#mwb_cf7_dependent_feed' ).val();
			
			if ( dependent.length > 0 ) {
				jQuery( '#mwb_' + crm + '_cf7_feeds_condition_meta_box' ).css( 'display', 'none' );
				jQuery( '#mwb_' + crm + '_cf7_feeds_add_note_meta_box' ).css( 'display', 'none' );
				jQuery( '#mwb_' + crm + '_cf7_feeds_manual_sync_meta_box' ).css( 'display', 'none' );
				jQuery( '#mwb_' + crm + '_cf7_feeds_add_attachment_meta_box' ).css( 'display', 'none' );
				jQuery( '#mwb-' + crm + '-cf7-select-form' ).attr( 'disabled', 'disabled' );
				jQuery( '#mwb-feeds-' + crm + '-object' ).attr( 'disabled', 'disabled' );
			} else {

				jQuery( '#mwb_' + crm + '_cf7_feeds_condition_meta_box' ).slideDown();
				jQuery( '#mwb_' + crm + '_cf7_feeds_add_note_meta_box' ).slideDown();
				jQuery( '#mwb_' + crm + '_cf7_feeds_manual_sync_meta_box' ).slideDown();
		
				switch ( objects ) {
					case 'Contact':
						jQuery( '#mwb_' + crm + '_cf7_feeds_associate_account_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_cf7_feeds_associate_campaign_meta_box' ).slideDown();
						break;

					case 'Lead':
						jQuery( '#mwb_' + crm + '_cf7_feeds_associate_account_meta_box' ).slideUp();
						jQuery( '#mwb_' + crm + '_cf7_feeds_associate_campaign_meta_box' ).slideDown();
						break;

					default :
						jQuery( '#mwb_' + crm + '_cf7_feeds_associate_account_meta_box' ).slideUp();
						jQuery( '#mwb_' + crm + '_cf7_feeds_associate_campaign_meta_box' ).slideUp();
						jQuery( '#mwb_' + crm + '_cf7_feeds_associate_contact_meta_box' ).slideDown();
				}
			}
		}
	}
	...
	const maybeApplySelect2 = () => {
		if ( jQuery( 'body' ).hasClass( 'post-type-mwb_' + crm + '_cf7' ) ) {
			let id = jQuery( '#mwb-' + crm + '-cf7-select-form, #mwb-feeds-' + crm + '-object' );
			jQuery( id ).select2();
		}
	}
	 ...
	 * 
	 * @param    {array} cf7fields    Form fields
	 * @since    1.0.1
	 * @returns  void
	 */
	const manageAttachmentFields = ( cf7fields ) => {
		if ( jQuery( 'body' ).hasClass( 'post-type-mwb_' + crm + '_cf7' ) ) {
			let selectfields = jQuery( '.mwb-' + crm + '-attachment-div' ).find( 'select' );

			let options = '';
			jQuery.each( cf7fields, function ( index, value ) {
    ...
    jQuery( 'input[name="mwb_setting[enable_notif]"]' ).on( 'change', function() {
        ...
        if ( email_enable ) {
            jQuery( '#mwb_' + crm + '_cf7_email_notif' ).removeClass( 'is_hidden' );
  
        } else {
            jQuery( '#mwb_' + crm + '_cf7_email_notif' ).addClass( 'is_hidden' );
        }
    } );
	...
	jQuery( '#mwb_cf7_integration_settings_form' ).on( 'submit', function( e ) {
		e.preventDefault();

		let data  = jQuery( this ).serialize();
		let event = 'save_general_setting';
		let input = {
			action : action,
			nonce  : nonce,
			data   : data,
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.status ) {
					triggerSuccess( response.data.message );
				} else {
	...
	jQuery( '#mwb-showauth-form' ).on( 'click', function( e ) {
		...
		jQuery( '.mwb-intro' ).slideUp( '400', function() {
			jQuery( '.mwb_cf7_integration_account-wrap' ).slideDown();
		} );
	} );
	...
	jQuery( '.mwb-section__sub-heading__wrap' ).on( 'click', function() {
		...
		jQuery( this ).siblings( '.mwb-cf7-integration__status-wrap' ).slideToggle( 300 );
	} );
	...
	jQuery( '#mwb-' + crm + '-cf7-authorize-button' ).on( 'click', function( e ) {
		e.preventDefault();
	
		let href          = jQuery( this ).attr( 'href' );
		let client_id     = jQuery( '#mwb-' + crm + '-cf7-client-id' ).val();
		let client_secret = jQuery( '#mwb-' + crm + '-cf7-client-secret' ).val();
		...
		if ( client_id.length == 0 || client_secret.length == 0 ) {
			...
			href = updateQueryStringParameter( href, 'client_secret', client_secret );
		}
	...
	jQuery( '#mwb_cf7_integration_refresh_token' ).on( 'click', function( e ) {

		let button = jQuery( this );
		let event  = 'refresh_crm_access_token';
		let input  = {
			action: action,
			nonce : nonce,
			event : event,
		...
		result.then( ( response ) => {
			...
			if ( true == response.status ) {
				if ( true == response.data.success ) {
					...
					jQuery( '#mwb-cf7-token-expiry-notice' ).text( response.data.token_message );
				} else {
					triggerError( response.data.msg );
				}
			} else {
	...
	/* Display reauth popup */
	jQuery( '#mwb_cf7_integration_reauthorize' ).on( 'click', function( e ) {
		e.preventDefault();

		jQuery( '.mwb-reauth__body' ).removeClass( 'row-hide' );
	} );

	/* Close reauth popup */
	jQuery( document ).on( 'click', '.mwb-reauth__body-close img', function( e ) {
		e.preventDefault();
		hideReauthPopup();
	} );

	/* Disconnect account */
	jQuery( '#mwb_cf7_integration_disconnect' ).on( 'click', function( e ) {
		e.preventDefault();

		let event = 'disconnect_account';
		let input = {
			action: action,
			nonce : nonce,
			event : event,
	...
	jQuery( '#mwb-' + crm + '-cf7-select-form' ).on( 'change', function() {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				...
				jQuery( '#mwb-' + crm + 'cf7-feed-add-note' ).slideUp();
				let note = createAddNoteField();
				jQuery( '#mwb-' + crm + 'cf7-feed-add-note' ).html( note );
				jQuery( '#mwb-' + crm + 'cf7-feed-add-note' ).slideDown();
	...
	jQuery( '#mwb-feeds-' + crm + '-object' ).on( 'change', function() {
		...
		if ( '-1' == jQuery( '#mwb-' + crm + '-cf7-select-form' ).val() ) {
		...
		result.then( ( response ) => {
			...
			if ( true == response.status ) {
				if ( response.hasOwnProperty( 'data' ) ) {
					if ( true == response.data.success ) {
						updateSelectOptions( select_id, response.data.data );
					} else {
	...
	jQuery( '#mwb-' + crm + '-refresh-fields' ).on( 'click', function() {
		...
		if ( '-1' == module ) {
			triggerError( 'Please select a Google Sheet first' );
			return;
		}
	...
	/* Get sheet tab fields on change */
	jQuery( document ).on( 'change', '#sheet-tab-field-select', function() {
	...
	jQuery( document ).on( 'click' , '#add-new-field-btn', function( e ) {
		...
		let object = jQuery( '#mwb-feeds-' + crm + '-object' ).val();
		let form   = jQuery( '#mwb-' + crm + '-cf7-select-form' ).val();
		let tab    = jQuery( '#sheet-tab-field-select' ).val();

		if( '-1' == object || '-1' == field ) {
			triggerInfo( 'Select a valid Google object and its corresponding field' );
			return;
		}
	...
	jQuery( document ).on( 'click', '.condition-or-btn', function( e ) {
		...
		let form = jQuery( '#mwb-' + crm + '-cf7-select-form' ).val();
		if ( '-1' == form ) {
			triggerInfo( 'Please select a valid CF7 form' );
			return;
		}
	...
	jQuery( '.mwb-feed-status' ).on( 'change', function() {
		...
		if ( this.checked ) {	
			...
			result.then( (response ) => {
				if ( true == response.status ) {
					jQuery( '.mwb-feed-status-text_' + feedId ).html( '<strong>Active</strong>' );
				}
			} );
			...
			result.then( (response ) => {
				if ( true == response.status ) {
					jQuery( '.mwb-feed-status-text_' + feedId ).html( '<strong>Sandbox</strong>' );
				}
			} );
		}
	} );
	...
	jQuery( '.mwb_cf7_integration_trash_feed' ).on( 'click', function() {
	...
	jQuery( document ).on( 'change', '.filter-feeds-by-form', function() {
		...
		if ( '-1' == form_id ) {
			triggerInfo( 'Select a valid CF7 form!!' );
			return;
		}
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.status ) {
					jQuery( '.mwb-cf7-integration__feed-list' ).slideUp();
					jQuery( '.mwb-cf7-integration__feed-list' ).html( response.data.feeds );
					jQuery( '.mwb-cf7-integration__feed-list' ).slideDown();
				}
			} else {
	...
	/* Toggle feeds fields */
	jQuery( '.mwb_cf7_integration__feeds-wrap .mwb-feeds__meta-box-main-wrapper' ).slideDown();
	jQuery( '.mwb_cf7_integration__feeds-wrap .mwb-feeds__header-link' ).addClass( 'link-active' );
	jQuery( document ).on( 'click', 'a.mwb-feeds__header-link', function() {
		jQuery( this ).toggleClass( 'link-active' );
		jQuery( this ).siblings( '.mwb-feeds__meta-box-main-wrapper' ).slideToggle( 300 );
	} );

	/* Remove required fields on condition enable toogle */
	jQuery( '#mwb-' + crm + '-enable-add-condition' ).on( 'change', function () {
	...
	/* Add new feed limit */
	jQuery( '.add_new_post' ).on( 'click', function( e ) {
		e.preventDefault();
		
		let href  = jQuery( this ).attr( 'href' );
		let event = 'limit_feeds_creation';
		let input = {
			action : action,
			nonce  : nonce,
			event  : event,
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.limit ) {
					triggerInfo( 'Feed limit reached!! Activate pro version to add unlimited feeds' );
				} else {
					window.open( href );
				}
			} else {
	...
	/* Clear Log */
	jQuery( '#mwb-' + crm + '-cf7-clear-log' ).on( 'click', function( e ) {
		e.preventDefault();
	...
	/* Download log */
	jQuery( '#mwb-' + crm + '-cf7-download-log' ).on( 'click', function( e ) {
		e.preventDefault();
