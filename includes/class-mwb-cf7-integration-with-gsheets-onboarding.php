 ...
 */
if ( class_exists( 'Mwb_Cf7_Integration_With_Gsheets_Onboarding' ) ) {
	return;
}
 ...
 */
class Mwb_Cf7_Integration_With_Gsheets_Onboarding {
	...
	public function __construct() {
		...
		if ( defined( 'MWB_CF7_INTEGRATION_WITH_GSHEETS_PLUGIN_NAME' ) ) {
			self::$plugin_name = MWB_CF7_INTEGRATION_WITH_GSHEETS_PLUGIN_NAME;
		}
	...
	public function enqueue_styles() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_style( 'makewebbetter-onboarding-style', MWB_CF7_INTEGRATION_WITH_GSHEETS_URL . 'admin/css/makewebbetter-onboarding-admin.min.css', array(), '1.0.0', 'all' );

			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_style( 'makewebbetter-onboarding-select2-style', MWB_CF7_INTEGRATION_WITH_GSHEETS_URL . 'packages/select2/select2.min.css', array(), '1.0.0', 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'makewebbetter-onboarding-scripts', MWB_CF7_INTEGRATION_WITH_GSHEETS_URL . 'admin/js/makewebbetter-onboarding-admin.min.js', array( 'jquery' ), '1.0.0', true );
			...
			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_script( 'makewebbetter-onboarding-select2-script', MWB_CF7_INTEGRATION_WITH_GSHEETS_URL . 'packages/select2/select2.min.js', array( 'jquery' ), '1.0.0', false );
		}
	}
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once MWB_CF7_INTEGRATION_WITH_GSHEETS_DIRPATH . 'extra-templates/makewebbetter-onboarding-template-display.php';
		}
	}
	...
	public function add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' == $pagenow ) { // phpcs:ignore
			require_once MWB_CF7_INTEGRATION_WITH_GSHEETS_DIRPATH . 'extra-templates/makewebbetter-deactivation-template-display.php';
		}
	}
	...
	public function is_valid_page_screen() {
		...
		if ( ! empty( $screen->id ) ) {
			...
			$is_valid = in_array( $screen->id, apply_filters( 'mwb_helper_valid_frontend_screens', array() ) ) && $this->add_mwb_additional_validation(); // phpcs:ignore
		}

		if ( empty( $is_valid ) && 'plugins.php' == $pagenow ) { // phpcs:ignore
			$is_valid = true;
		}
	...
	public function can_show_onboarding_popup() {
		...
		$is_already_sent = get_option( 'mwb-' . $this->crm_slug . '-cf7-onboarding-data-sent', false );

		// Already submitted the data.
		if ( ! empty( $is_already_sent ) && 'sent' == $is_already_sent ) { // phpcs:ignore
			return false;
		}

		$get_skipped_timstamp = get_option( 'mwb-' . $this->crm_slug . '-cf7-onboarding-data-skipped', false );
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			wp_rand() => array(
				'id'          => 'monthly-revenue',
				'label'       => esc_html__( 'What is your monthly revenue?', 'mwb-cf7-integration-with-google-sheets' ),
				'type'        => 'radio',
			...
			wp_rand() => array(
				'id'          => 'industry_type',
				'label'       => esc_html__( 'What industry defines your business?', 'mwb-cf7-integration-with-google-sheets' ),
				'type'        => 'select',
				'name'        => 'industry_type_',
				'value'       => '',
				'multiple'    => 'yes',
			...
			wp_rand() => array(
				'id'          => 'onboard-email',
				'label'       => esc_html__( 'What is the best email address to contact you?', 'mwb-cf7-integration-with-google-sheets' ),
				'type'        => 'email',
				'name'        => 'email',
			...
			wp_rand() => array(
				'id'          => 'onboard-number',
				'label'       => esc_html__( 'What is your contact number?', 'mwb-cf7-integration-with-google-sheets' ),
				'type'        => 'text',
				'name'        => 'phone',
				'value'       => '',
				'required'    => 'yes',
			...
			wp_rand() => array(
				...
				'type'        => 'hidden',
				'name'        => 'show-counter',
				'value'       => get_option( 'mwb-' . $this->crm_slug . '-cf7-onboarding-data-sent', 'not-sent' ),
				'required'    => '',
				'extra-class' => '',
			),
	...
	public function render_field_html( $attr = array(), $base_class = 'on-boarding' ) {
		...
		$multiple = ! empty( $attr['multiple'] ) && 'yes' == $attr['multiple'] ? 'yes' : 'no'; // phpcs:ignore
		$required = ! empty( $attr['required'] ) ? 'required="required"' : '';

		$html = '';

		if ( 'hidden' != $type ) : // phpcs:ignore ?>
		...
		switch ( $type ) {
			...
			case 'radio':
				...
				if ( ! empty( $options ) && is_array( $options ) ) :
					...
					<?php
					$is_multiple = ! empty( $multiple ) && 'yes' != $multiple ? 'name = "' . $name . '"' : ''; // phpcs:ignore
			...
			case 'select2':
				...
				if ( ! empty( $options ) && is_array( $options ) ) {
					...
					$is_multiple = 'yes' == $multiple ? 'multiple' : ''; // phpcs:ignore
					$select2     = ( 'yes' == $multiple && 'select' == $type ) || 'select2' == $type ? 'on-boarding-select2 ' : ''; // phpcs:ignore
					?>
					...
					<select class="on-boarding-select-field <?php echo esc_html( $select2 ); ?> <?php echo esc_html( $class ); ?>" id="<?php echo esc_html( $id ); ?>" name="<?php echo esc_html( $name ); ?>[]" <?php echo esc_html( $required ); ?> <?php echo esc_html( $is_multiple ); ?>>
						...
						<?php if ( 'select' == $type ) : // phpcs:ignore ?>	
							<option class="on-boarding-options" value=""><?php esc_html( 'Select Any One Option...', 'mwb-cf7-integration-with-google-sheets' ); ?></option>
						<?php endif; ?>
	...
	public function send_onboarding_data() {
		...
		if ( ! empty( $form_data ) && is_array( $form_data ) ) {
			...
			foreach ( $form_data as $key => $input ) {
				...
				if ( 'show-counter' == $input->name ) { // phpcs:ignore
					continue;
				}
		...
		if ( ! empty( $action_type ) && 'onboarding' == $action_type ) { // phpcs:ignore
			$get_skipped_timstamp = update_option( 'mwb-' . $this->crm_slug . '-cf7-onboarding-data-sent', 'sent' );
		}
	...
	public function skip_onboarding_popup() {
		...
		$get_skipped_timstamp = update_option( 'mwb-' . $this->crm_slug . '-cf7-onboarding-data-skipped', time() );
	 ...
	 *
	 * @param      array  $submission Bool value to check form submission.
	 * @param      string $action_type Action of user.
	 * @since      1.0.0
	 * @return     bool
	 */
	...
	private function hic_post( $endpoint, $post_params, $headers ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$status_code = 500;
			$_response   = esc_html__( 'Unexpected Error Occured', 'mwb-cf7-integration-with-google-sheets' );
			$errors      = $response;
	...
	/**
	 *  Hubwoo Onboarding Submission ::submit form data.
	 *
