 ...
 * @subpackage Mwb_Cf7_Integration_With_Gsheets/public
 */
 ...
 */
class Mwb_Cf7_Integration_With_Gsheets_Public {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$request_class        = 'Mwb_Cf7_Integration_' . $this->crm_name . '_Request_Module';
	...
	public static function get_instance() {
		...
		if ( null == self::$_instance ) { //phpcs:ignore
			...
			self::$_instance = new self( 'mwb-cf7-integration-with-gsheets', MWB_CF7_INTEGRATION_WITH_GSHEETS_VERSION );
		}
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/mwb-cf7-integration-with-gsheets-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/mwb-cf7-integration-with-gsheets-public.js', array( 'jquery' ), $this->version, false );
	...
	public function mwb_cf7_integration_send_to_crm( $data = array() ) {
		...
		$api          = 'Mwb_Cf7_Integration_' . $this->crm_name . '_Api';
		...
		if ( ! empty( $active_feeds ) && is_array( $active_feeds ) ) {
			foreach ( $active_feeds as $key => $feed_id ) {
				...
				$sheet_tab    = $this->fetch_feed_data( $feed_id, 'mwb-' . $this->crm_slug . '-cf7-tab-field', '' );

				$log_data = array(
					'feed_id'      => $feed_id,
					'feed_name'    => get_the_title( $feed_id ),
					'google_sheet' => $crm_object,
				);
				...
				if ( ! empty( $filter_exist ) ) {
					...
					if ( true === $filter_result ) { // If filter results true, then send data to CRM.
						...
						$result = $api_module->handle_single_record( $crm_object, $sheet_tab, $request, $log_data );
	...
	public function mwb_cf7_integration_send_email( $info, $data ) {
		...
		if ( ! empty( $info ) && is_array( $info ) ) {
			...
			$subject   = sprintf( esc_html__( 'Error While Posting CF7 form data over %s ', 'mwb-cf7-integration-with-google-sheets' ), esc_html( $this->crm_name ) );
			...
			if ( isset( $info['data'] ) && isset( $info['data'] ) ) {
				$info_msg    = $info['data']['error']['message'];
				$info_detail = is_array( $info['data'] ) ? wp_json_encode( $info['data'] ) : $info['data'];
			}

			$email_data = array(
				'Title'          => esc_html__( 'MWB CF7 Integration with Insightly', 'mwb-cf7-integration-with-google-sheets' ),
