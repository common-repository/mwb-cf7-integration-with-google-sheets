 ...
 */
class Mwb_Cf7_Integration_Gsheets_Framework {
		...
		if ( ! empty( $form_tags ) && is_array( $form_tags ) ) {
			...
			foreach ( $form_tags as $tag ) {
				if ( $admin::is_pro_available_and_active() ) {
					if ( 'tel' == $tag->basetype || 'file' == $tag->basetype ) { //phpcs:ignore
						continue;
					} elseif ( 'number' == $tag->basetype ) { // phpcS:ignore
						$min = '';
						$max = '';
						foreach ( $tag->options  as $key => $value ) {
							if ( false !== strpos( $value, 'min' ) ) {
								$min = substr( $value, 4 );
							}
							if ( false !== strpos( $value, 'max' ) ) {
								$max = substr( $value, 4 );
							}
						}
						if ( $min >= 10 ) {
	...
	public function get_avialable_form_filters() {
		...
		$filter = array(
			'exact_match'       => esc_html__( 'Matches exactly', 'mwb-cf7-integration-with-google-sheets' ),
			'no_exact_match'    => esc_html__( 'Does not match exactly', 'mwb-cf7-integration-with-google-sheets' ),
			'contains'          => esc_html__( 'Contains (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'not_contains'      => esc_html__( 'Does not contain (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'exist'             => esc_html__( 'Exist in (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'not_exist'         => esc_html__( 'Does not Exists in (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'starts'            => esc_html__( 'Starts with (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'not_starts'        => esc_html__( 'Does not start with (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'ends'              => esc_html__( 'Ends with (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'not_ends'          => esc_html__( 'Does not end with (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'less_than'         => esc_html__( 'Less than (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'greater_than'      => esc_html__( 'Greater than (Text)', 'mwb-cf7-integration-with-google-sheets' ),
			'less_than_date'    => esc_html__( 'Less than (Date/Time)', 'mwb-cf7-integration-with-google-sheets' ),
			'greater_than_date' => esc_html__( 'Greater than (Date/Time)', 'mwb-cf7-integration-with-google-sheets' ),
			'equal_date'        => esc_html__( 'Equals (Date/Time)', 'mwb-cf7-integration-with-google-sheets' ),
			'empty'             => esc_html__( 'Is empty', 'mwb-cf7-integration-with-google-sheets' ),
			'not_empty'         => esc_html__( 'Is not empty', 'mwb-cf7-integration-with-google-sheets' ),
		);
	...
	public function is_crm_connected() {
		$connect = get_option( 'mwb-cf7-' . $this->crm_slug . '-crm-active', false );
	...
	public function get_available_crm_objects() {
		...
		$not_connected = esc_html__( 'Application is not connected to Google Sheets', 'mwb-cf7-integration-with-google-sheets' );

		$crm_objects = $this->crm_api_module->get_modules_data();
		if ( is_array( $crm_objects ) && empty( $crm_objects ) ) {
			$not_connected = esc_html__( 'No Spreadsheets Found, Create a Spreadsheet first.', 'mwb-cf7-integration-with-google-sheets' );
		}
	...
	public function get_crm_token_details( $detail = false ) {
		...
		switch ( $detail ) {
			...
			case 'account_owner':
				$output = $this->crm_api_module->get_account_owner_name();
				break;

			case 'account_email':
				$output = $this->crm_api_module->get_account_owner_email();
				break;
		}
