 ...
 */
class Mwb_Cf7_Integration_Gsheets_Request_Module {
	...
	/**
	 * Main Mwb_Cf7_Integration_Salesforce_Request_Module Instance.
	 *
	 * Ensures only one instance of Mwb_Cf7_Integration_Salesforce_Request_Module is loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Mwb_Cf7_Integration_Salesforce_Request_Module - Main instance.
	 */
	...
	public function get_crm_request( $crm_obj = false, $feed_id = false, $entries = array() ) {
		...
		// Restructure request as per Gsheet.
		$crm_fields = $this->get_feed( $feed_id, 'mwb-' . $this->crm_slug . '-cf7-crm-fields' );
		$crm_fields = json_decode( $crm_fields, true );

		$result = array();
		if ( is_array( $crm_fields ) ) {
			foreach ( $crm_fields as $key => $field ) {
				$result[ substr( $key, -1 ) ] = isset( $response[ $key ] ) ? $response[ $key ] : null;
			}
		}

		$result = apply_filters( 'mwb_' . $this->crm_slug . '_cf7_request_data', $result, $crm_obj, $feed_id, $entries );
