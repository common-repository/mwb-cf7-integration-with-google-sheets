<?php
/**
 * The complete management for the Salesforce-CF7 feeds custom post type.
 *
 * @since      1.0.0
 ...
 * @author     MakeWebBetter <https://makewebbetter.com>
 */
class Mwb_Cf7_Integration_Gsheets_Feed_Module {
	...
	/**
	 * Main Mwb_Cf7_Integration_Salesforce_Feed_Module Instance.
	 *
	 * Ensures only one instance of Mwb_Cf7_Integration_Salesforce_Feed_Module is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 *
	 * @static
	 * @return Mwb_Cf7_Integration_Salesforce_Feed_Module - Main instance.
	 */
	...
	public function register_feeds_post_type() {
		...
		$labels = array(
			'name'               => _x( 'Feeds', 'Post Type General Name', 'mwb-cf7-integration-with-google-sheets' ),
			'singular_name'      => _x( 'Feed', 'Post Type Singular Name', 'mwb-cf7-integration-with-google-sheets' ),
			'menu_name'          => _x( 'Crm Feeds', 'Admin menu name', 'mwb-cf7-integration-with-google-sheets' ),
			'parent_item_colon'  => __( 'Parent Feed', 'mwb-cf7-integration-with-google-sheets' ),
			'all_items'          => __( 'All Feeds', 'mwb-cf7-integration-with-google-sheets' ),
			'view_item'          => __( 'View Feed', 'mwb-cf7-integration-with-google-sheets' ),
			'add_new_item'       => __( 'Add New Feed', 'mwb-cf7-integration-with-google-sheets' ),
			'add_new'            => __( 'Add New', 'mwb-cf7-integration-with-google-sheets' ),
			'edit_item'          => __( 'Edit Feed', 'mwb-cf7-integration-with-google-sheets' ),
			'update_item'        => __( 'Update Feed', 'mwb-cf7-integration-with-google-sheets' ),
			'search_items'       => __( 'Search Feed', 'mwb-cf7-integration-with-google-sheets' ),
			'not_found'          => __( 'Not Found', 'mwb-cf7-integration-with-google-sheets' ),
			'not_found_in_trash' => __( 'Not found in Trash', 'mwb-cf7-integration-with-google-sheets' ),
		);

		$args = array(
			'label'                => __( 'Feeds', 'mwb-cf7-integration-with-google-sheets' ),
			'description'          => __( 'Feeds for crm', 'mwb-cf7-integration-with-google-sheets' ),
			'labels'               => $labels,
	...
	public function add_feed_meta_box() {
		...
		$metaboxes[] = array(
			...
			'title'    => esc_html__( 'Feed details', 'mwb-cf7-integration-with-google-sheets' ),
		...
		$metaboxes[] = array(
			...
			'title'    => esc_html__( 'Conditional Statements', 'mwb-cf7-integration-with-google-sheets' ),
		...
		foreach ( $metaboxes as $key => $metabox ) {
			...
			if ( class_exists( 'Cf7_Integration_With_' . $this->crm_name . '_Feed_Extension' ) && ! method_exists( $this, $metabox['callback'] ) ) {
	...
	public function get_pro_callback( $key = '' ) {
		...
		switch ( $key ) {
			...
			case 'pro-add-options':
				$result = array( $pro_class, 'mwb_cf7_' . $this->crm_slug . '_feeds_pro_add_options' );
				break;
	...
	public function feeds_mb_condition_render() {
		...
		$params['enable_filter'] = $this->fetch_feed_data( $post->ID, 'mwb-' . $this->crm_slug . '-cf7-enable-filters', '' );
	...
	public function save_feeds_data( $post_id ) {
		...
		if ( isset( $_POST['post_type'] ) && $this->feed_name == $_POST['post_type'] ) { // phpcs:ignore
			...
			$tab_field        = isset( $_POST['tab_field'] ) ? sanitize_text_field( wp_unslash( $_POST['tab_field'] ) ) : '';
			$crm_fields       = isset( $_POST['mwb-gsheet-fields'] ) ? map_deep( wp_unslash( $_POST['mwb-gsheet-fields'] ), 'sanitize_text_field' ) : array();
			...
			update_post_meta( $post_id, 'mwb-' . $this->crm_slug . '-cf7-tab-field', $tab_field );
			update_post_meta( $post_id, 'mwb-' . $this->crm_slug . '-cf7-enable-filters', $enable_filters );
			update_post_meta( $post_id, 'mwb-' . $this->crm_slug . '-cf7-condtion-field', $condition );
			update_post_meta( $post_id, 'mwb-' . $this->crm_slug . '-cf7-crm-fields', $crm_fields );
	...
	/**
	 * Get selected google sheet.
	 *
	 * @param      string $sheet_id       ID of the google sheet.
	 * @since      1.0.0
	 * @retruns    string
	 */
	public function get_google_sheet( $sheet_id ) {

		if ( empty( $sheet_id ) ) {
			return;
		}

		$sheets     = $this->connect_manager->get_available_crm_objects();
		$sheet_name = '';

		if ( ! empty( $sheets ) ) {
			foreach ( $sheets as $key => $value ) {
				if ( ! empty( $value['id'] && ! empty( $value['name'] ) ) ) {
					if ( $sheet_id == $value['id'] ) { // phpcs:ignore
